Mangroves_Herbivory=DATA_PREP$new(
	...
	format=function(inpDF) {
		inpDF %>% setnames(c("group","zonal point","TapePoint","quarter","stems","% of leaf missing","leaf distorted ?","number of Mines","number of Galls", "location notes") ,c("Group","Zonal point","Tape point","Quarter","Stem","Percent of leaf missing","Leaf distorted","Number of Mines","Number of Galls", "Location notes"))
		inpDF %>% mutate(
				Longitude=na.pad(Longitude),
				Latitude=na.pad(Latitude),
	...
	errors=list(
		ERROR_TEMPLATE.FACTOR("Zonal point", c('Seaward','Landward - Creek')),
		ERROR_TEMPLATE.FACTOR("Intertidal zone", c('High','Low')),
		...
		ERROR_TEMPLATE.BINARY("Bud 1 Live"),
		ERROR_TEMPLATE.BINARY("Bud 2 Live"),
		ERROR_TEMPLATE.BINARY("Bud 3 Live"),
	...
	process=function(inpDF, ...) {
		...
		stemDF=inpDF[,
			...
			by=list(Group, `Zonal point`, `Intertidal zone`, `Tape point`, Quarter, Stem)
		]
		quarterDF=stemDF[,
			list(
				`Date`=na.omit(first(Date)),
			...
			by=list(Group, `Zonal point`, `Intertidal zone`, `Tape point`, Quarter)
		]
		return(quarterDF)
	}
)