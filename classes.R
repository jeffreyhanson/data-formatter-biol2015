...
MANAGER=setRefClass("MANAGER",
	...
	methods=list(
		...
		saveDataToFile=function() {
			...
			# save cleaned data
			write.table(
				dplyr::select(.activeGroupData_DF, -DeleteButton, -Row),
			...
			write.table(
				mutate(
					transform(.dataPrep$processData(dplyr::select(.activeGroupData_DF, -DeleteButton, -Row), omitRows=.omittedRows_BOL), Week=.activeWeekNumber_CHR),
			...
			# if community dataset then save special summaries
			if (.activeProjectName_CHR=='Mangroves_Community') {
				subDF = masterDF %>% filter(Frequency>0)
				# species transect zone `relative importance` - subset to values greater then zero
				write.table(
					subDF[, .(`Relative Importance`=mean(`Relative Importance`)), by=.(Transect, Zone, Species)],
					file.path("master", .activeProjectName_CHR, paste0("impdata_",format(Sys.time(), '%Y-%m-%d_%H-%M-%S'),".csv")),
					sep=",",
					row.names=FALSE
				)
			
				# # species zone transect -- unique species names
				# write.table(
					# subDF[,.(Species=unique(Species)), by=.(Transect, Zone)],
					# file.path("master", .activeProjectName_CHR, paste0("freqdata_",format(Sys.time(), '%Y-%m-%d_%H-%M-%S'),".csv")),
					# sep=",",
					# row.names=FALSE
				# )
			}
